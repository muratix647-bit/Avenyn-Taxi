...
export default function Page() {
  ...
  async function loadEverything(userId: string) {
    ...
    const actualRole = (profile?.role || "driver") as "admin" | "driver";
    setRole(actualRole);
    ...
    if (actualRole === "admin") {
      ...
      if ((driversRes.data || []).length > 0 && !tripDriver) {
        setTripDriver((driversRes.data as Driver[])[0].id);
      }

      if ((driversRes.data || []).length > 0 && !selectedLinkDriverId) {
        setSelectedLinkDriverId((driversRes.data as Driver[])[0].id);
      }

      if ((profilesRes.data || []).length > 0 && !selectedUser) {
        setSelectedUser((profilesRes.data as Profile[])[0].id);
      }
    ...
    setCurrentDriver((driver as Driver) || null);
    setDrivers(driver ? [driver as Driver] : []);
    setProfiles([]);
    setTripDriver(driver?.id || "");
  ...
  async function changeRole(userId: string, newRole: "admin" | "driver") {
    try {
      setSaving(true);
      setPageError("");
      setSuccessMessage("");
      ...
      setSuccessMessage(`Roll uppdaterad till ${newRole}.`);
                  ...
                  profiles.map((p) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-sm text-slate-400">Roll: {p.role}</div>
                      </div>
